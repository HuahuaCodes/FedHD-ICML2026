# import sys
# sys.path.append('/scratch/iq24/cc0395/FedWSIDD')
import torch
...
def define_clam(args):
    ...
    if args.mil_method != 'mil':
        if args.ft_model in ['ResNet50', 'UNI',  'PhikonV2', 'GPFM']:
            model_size = 'small'
...
FOUNDATION_MODELS_1024 = ['ResNet50', 'UNI', 'PhikonV2', 'GPFM']

def _resolve_model_size(ft_model):
    if any(m in ft_model for m in FOUNDATION_MODELS_1024):
        return 'small'
    elif 'ViT_S_16' in ft_model:
        return 'ultra_small'
    elif 'ViT_T_16' in ft_model:
        return 'tiny'
    else:
        raise NotImplementedError(f'Unknown feature extractor: {ft_model}')

def define_transmil(args):
    model_size = _resolve_model_size(args.ft_model)
