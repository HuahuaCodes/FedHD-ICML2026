...
def save_syn_img(image_syns, results_dir, iter=0, client_idx=None, G=None):
    STDS = [0.229, 0.224, 0.225]
    MEANS = [0.485, 0.456, 0.406]
    if G is not None:
        image_syns = G.gen_image(image_syns).unsqueeze(0)
    ...
    for slide_i in range(len(image_syns)):
        ...
        # print('Save image size ', image_syn_vis.size())
        for i in range(image_syn_vis.size(1)):  # Iterate over the second dimension (channels)
            image_syn_vis[:, i, :, :] = image_syn_vis[:, i, :, :] * STDS[i] + MEANS[i]
        image_syn_vis[image_syn_vis < 0] = 0.0
        image_syn_vis[image_syn_vis > 1] = 1.0
...
def save_syn_slide_image(image_syns, results_dir, iter=0, client_idx=None):
    STDS = [0.229, 0.224, 0.225]
    MEANS = [0.485, 0.456, 0.406]
    if client_idx is None:
        # iterate over clients
        ...
        for slide_i in range(len(image_syns)):
            save_name = os.path.join(save_client, f'CLAM_DM_slide_{slide_i}.png')
            image_syn_vis = deepcopy(image_syns[slide_i].detach().cpu())
            image_syn_vis = image_syn_vis.unsqueeze(0)
            print('Save image size ', image_syn_vis.size())
            for i in range(image_syn_vis.size(1)):  # Iterate over the second dimension (channels)
                image_syn_vis[:, i, :, :] = image_syn_vis[:, i, :, :] * STDS[i] + MEANS[i]
            image_syn_vis[image_syn_vis < 0] = 0.0
