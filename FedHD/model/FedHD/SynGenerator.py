...
def load_img_from_pth(args, img_pth, class_proto):
    # Load feature tensor; h5 sampling_prob not available for all extractors.
    X = torch.load(img_pth).reshape((-1, args.syn_size))
    X_prob = torch.ones(X.shape[0])
    N = X.shape[0]
    if args.use_adaptive_sampling:
        h5_pth = img_pth.replace("/pt_files/", "/h5_files/").replace(".pt", ".h5")
        with h5py.File(h5_pth, "r") as file:
            if "sampling_prob" in file:
                X_prob = torch.from_numpy(np.array(file["sampling_prob"][:])).float()
                if not torch.isclose(X_prob.sum(), torch.tensor(1.0), atol=1e-2):
                    X_prob = X_prob / X_prob.sum()
        if len(X_prob) != N:
            X_prob = torch.ones(N)
        num_sampled = min(int(N * args.scale_factor), N)
        sampled_indices = torch.multinomial(X_prob, num_sampled, replacement=True)
        X = X[sampled_indices]
        X_prob = X_prob[sampled_indices]
...
class FedWSIDDG:
    ...
    def run(self, repeat):
        ...
        for client_idx in range(len(self.n_clients)):
            ...
            # Free previous client syn tensor from GPU to save memory
            if client_idx > 0:
                image_syns[client_idx - 1] = image_syns[client_idx - 1].detach().cpu()
            ...
            if self.args.instance_learn:
                ...
                for i in range(len(images_all)):
                    ...
                    cached_images.append(image_real.cpu())
                    cached_probs.append(image_real_prob.cpu())
                ...
                if self.args.instance_learn:
                    ...
                    for batch_idx in range(num_batches):
                        ...
                        for i in range(start_idx, end_idx):
                            image_real = cached_images[i].to(self.device)
        ...
        # '''Final training (CBF: Stage1=real+CE, Stage2=real+synthetic+GCE)'''
        ...
        for client_idx in range(len(self.n_clients)):
            if self.args.heter_model:
                self.args.ft_model   = self.client_ft_models[client_idx]
                self.args.mil_method = self.client_mil_methods[client_idx]
            real_train_loader = self.get_train_loader(self.train_dataset[client_idx])
            syn_train_loader  = concated_train_loaders[client_idx]
            ...
            for ep in range(self.args.local_epochs * 2):
                batch_loss = 0.
                batch_error = 0.
                if ep < self.args.local_epochs:
                    # Stage 1: real data only, CE
                    for batch_idx, (images, labels) in enumerate(real_train_loader):
                        if len(images.shape) == 3 and images.shape[0] == 1:
                            images = images.squeeze(0)
                        images, labels = images.to(self.device), labels.to(self.device)
                        labels = labels.long()
                        optimizer_client.zero_grad()
                        loss, error, y_prob = self.mil_run(clients_models_pre[client_idx], images, labels, self.mil_loss)
                        loss.backward()
                        optimizer_client.step()
                        batch_loss += loss.item()
                    active_loader = real_train_loader
                else:
                    # Stage 2: real + cross-client synthetic, GCE
                    for batch_idx, (images, labels) in enumerate(syn_train_loader):
                        images, labels = images.to(self.device), labels.to(self.device)
                        labels = labels.long()
                        optimizer_client.zero_grad()
                        ...
                        optimizer_client.step()
                        batch_loss += loss.item()
                    active_loader = syn_train_loader

                batch_loss /= max(len(active_loader), 1)
                batch_error /= max(len(active_loader), 1)
