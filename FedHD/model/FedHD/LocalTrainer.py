...
from utils.Get_data import define_data, MixupDataset
...
from model.FedHD.distillation import distribution_matching, distribution_matching_woMIL, distribution_matching_woMIL_wG, distribution_matching_woMIL_latent, distribution_matching_latent_instance
...
# --- Heterogeneous model pool (paper §4.3, Table 1) ---
EXTRACTOR_POOL = ['R50_features', 'UNI_features', 'PhikonV2_features', 'GPFM_features']
MIL_POOL       = ['CLAM_SB', 'TransMIL', 'ACMIL']
TASK_N_CLIENTS = {'CAMELYON16': 2, 'CAMELYON17': 5, 'IDH': 8}

# Fixed per-client configs from Table 1 (feature_type, mil_method)
FT_MODEL_MAP = {
    "R50_features":      "ResNet50",
    "UNI_features":      "UNI",
    "PhikonV2_features": "PhikonV2",
    "GPFM_features":     "GPFM",
}

PAPER_CONFIGS = {
    'CAMELYON16': [
        ('R50_features',      'CLAM_SB'),
        ('UNI_features',      'TransMIL'),
    ],
    'CAMELYON17': [
        ('UNI_features',      'CLAM_SB'),
        ('R50_features',      'TransMIL'),
        ('R50_features',      'ACMIL'),
        ('PhikonV2_features', 'TransMIL'),
        ('GPFM_features',     'CLAM_SB'),
    ],
    'IDH': [
        ('R50_features',      'CLAM_SB'),
        ('GPFM_features',     'TransMIL'),
        ('R50_features',      'ACMIL'),
        ('UNI_features',      'ACMIL'),
        ('PhikonV2_features', 'CLAM_SB'),
        ('GPFM_features',     'CLAM_SB'),
        ('R50_features',      'ACMIL'),
        ('PhikonV2_features', 'CLAM_SB'),
    ],
...
def _get_n_clients(task):
    for key in TASK_N_CLIENTS:
        if key in task:
            return TASK_N_CLIENTS[key]
    raise ValueError(f'Unknown task: {task}')

def _assign_hetero_configs(task, repeat=0):
    """Return (feature_type_list, mil_method_list) matching Table 1."""
    for key in PAPER_CONFIGS:
        if key in task:
            pairs = PAPER_CONFIGS[key]
            return [p[0] for p in pairs], [p[1] for p in pairs]
    raise ValueError(f'No paper config for task: {task}')
class FedWSIDDM:
    def __init__(self, args, logger):
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.args = args
        self.logger = logger
        # Assign per-client (feature_extractor, MIL) configs
        if self.args.heter_model:
            ft_list, mil_list = _assign_hetero_configs(args.task, getattr(args, 'rep', 0))
            self.client_feature_types = ft_list
            self.client_ft_models     = [FT_MODEL_MAP[ft] for ft in ft_list]
            self.client_mil_methods   = mil_list
        else:
            n = _get_n_clients(args.task)
            self.client_feature_types = [args.feature_type] * n
            self.client_ft_models     = [args.ft_model] * n
            self.client_mil_methods   = [args.mil_method] * n
        feature_type_list = self.client_feature_types if self.args.heter_model else None
        self.train_dataset, self.test_dataset, self.n_clients = define_data(args, logger, feature_type_list=feature_type_list,
        ...
        for ep in range(self.args.local_epochs):
            ...
            if ep % 10 == 0:
                if test_loader is not None:
                    ...
                    # self.logger.info(f'Agent: {agent_idx}, Iter: {ep}, Loss: {current_epoch_loss}, Test Acc: {1-total_error}')
                    ...
                    self.logger.info(f'Agent: {agent_idx}, Iter: {ep}, Loss: {current_epoch_loss}')
        if test_loader is not None:
    ...
    def pretrain_clients(self, repeat, save_best=False):
        ...
        for idx in range(len(self.n_clients)):
            if self.args.heter_model:
                self.args.ft_model    = self.client_ft_models[idx]
                self.args.mil_method  = self.client_mil_methods[idx]
                print(f'=====> Client {idx}: extractor={self.args.ft_model}  MIL={self.args.mil_method}')
    ...
    def init_synthetic_data(self):
        if self.args.use_generate_prior or self.args.use_latent_prior:
            # syn data as latent embedding + G for mapping to image
            from model.s3GAN.s3ngan import singledecoderGANModel
            G = singledecoderGANModel()
            G = G.eval().requires_grad_(False).to(self.device)
             # init as random noise, then use Generator to generate synthetic data
            image_syns = [torch.randn(size=(self.args.n_classes*self.args.ipc, self.args.nps, self.args.syn_size), #(classes*ipc, nps, z_dim) G.netG.z_dim
                                      dtype=torch.float,
                                      requires_grad=True,
    ...
    def run(self, repeat):
        ...
        if self.args.load_syn_data:
            self.logger.info('=========================Loading synthetic data=========================')
            assert os.path.exists(f'{self.args.syn_data_dir}'), 'synthetic data not found'
            syn_data_iter = [sorted(map(int, os.listdir(f'{self.args.syn_data_dir}/0/client{client_idx}')))[-1] for client_idx in range(len(self.n_clients))]
            syn_image_ft = []
            for client_idx in range(len(self.n_clients)):
                syn_image_ft.append(torch.load(f'{self.args.syn_data_dir}/0/client{client_idx}/{syn_data_iter[client_idx]}/synthetic_images.pt'))
                self.logger.info(f'Client {client_idx} - Shape of syn_image_ft: {syn_image_ft[client_idx].shape}')
        else:
            ...
            for client_idx in range(len(self.n_clients)):
                ...
                for it in pbar_dd:
                    ...
                    image_real_prob = []
                    for c in range(len(indices_class)):
                        ft, prob = get_images(self.args, images_all, indices_class, c, image_batch, class_proto[c])
                    ...
                    if it % self.args.test_iter == 0 or it == inv_iters - 1:
                        ...
                        print('Perform synthetic data training on client ', client_idx, 'with ', len(virtual_dataset), ' virtual images')
                        if self.args.heter_model:
                            self.args.ft_model   = self.client_ft_models[client_idx]
                            self.args.mil_method = self.client_mil_methods[client_idx]
        ...
        # global_virtual_weights = [client_dd_acc[client_idx]/clients_models_acc[client_idx] for client_idx in
        #                          range(len(self.n_clients))]
        # perform feature augmentation using global virtual data from other clients
        # global_virtual_proto = []
        # # mixup_rate = []
        # with torch.no_grad():  
        #     for client_idx in range(len(self.n_clients)):
        #         local_virtual_proto = []
        #         for label in range(self.args.n_classes):
        #             same_label_virtual_ft = global_virtual_fts[client_idx][global_virtual_labels[client_idx] == label].reshape(-1, 1024).to(self.device)
        #             same_label_virtual_ft = same_label_virtual_ft.reshape(-1, self.args.nps, 1024)
        #             local_virutal_proto_per_cls = []
        #             for i in range(same_label_virtual_ft.shape[0]):
        #                 _, _, _, proto_feature = self.mil_run(clients_models_pre[client_idx], same_label_virtual_ft[i], torch.tensor([label]).to(self.device), self.mil_loss, return_feature=True)
        #                 local_virutal_proto_per_cls.append(proto_feature)
        #             local_virutal_proto_per_cls = torch.mean(torch.cat(local_virutal_proto_per_cls, dim=0), dim=0, keepdim=True)
        #             local_virtual_proto.append(local_virutal_proto_per_cls)
        #         local_virtual_proto = torch.cat(local_virtual_proto, dim=0)
        #         local_virtual_proto = local_virtual_proto.unsqueeze(0)
        #         global_virtual_proto.append(local_virtual_proto)
        # global_virtual_proto = torch.cat(global_virtual_proto, dim=0)
        # print('Virtual proto size:', global_virtual_proto.shape)
            ...
            for ep in range(local_enhanced_epoch):
                ...
                if ep < self.args.local_epochs:
                    ...
                    for batch_idx, (images, labels) in enumerate(concated_train_loaders[client_idx]):
                        ...
                        labels = labels.long()
                        optimizer_client.zero_grad()
                        loss, error, y_prob = self.mil_run(clients_models_pre[client_idx], images[0], labels, gce_loss_fn)
                        loss.backward()
                        optimizer_client.step()
                        batch_loss += loss.item()
                    # if batch_idx % 20 == 0:
