...
class FedCamelyon17Pat(Camelyon17RawPat):
    ...
    def train_test_split(self, train_ratio, deterministic):
        ...
        # Group patients by their primary class (most common label in their slides)
        class_to_patients = {0: [], 1: [], 2: [], 3: []}
        for pat_name in all_patients:
            pat_slides = self.center_pat_slide[pat_name]
            class_to_patients[0].append(pat_name)
        # Shuffle patients within each class
        for class_id in class_to_patients:
            random.shuffle(class_to_patients[class_id])
        
        # Split patients per class ensuring at least 1 patient in both train and test
        train_patients = []
        test_patients = []
        
        for class_id in class_to_patients:
            patients_in_class = class_to_patients[class_id]
            n_patients = len(patients_in_class)
            
            if n_patients == 0:
                continue
            elif n_patients == 1:
                # If only 1 patient, put in train
                train_patients.append(patients_in_class[0])
            else:
                # Ensure at least 1 in each set
                n_train_class = max(1, int(n_patients * train_ratio))
                n_test_class = n_patients - n_train_class
                
                # Adjust if test would be empty
                if n_test_class == 0:
                    n_test_class = 1
                    n_train_class = n_patients - 1
                
                train_patients.extend(patients_in_class[:n_train_class])
                test_patients.extend(patients_in_class[n_train_class:])
        
        # Convert patient names to indices
        patient_to_idx = {pat: idx for idx, pat in enumerate(all_patients)}
        train_indices = [patient_to_idx[pat] for pat in train_patients]
        test_indices = [patient_to_idx[pat] for pat in test_patients]
