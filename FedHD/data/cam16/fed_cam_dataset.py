...
class Camelyon16Raw(Dataset):
    def __init__(
        ...
        if syn_dataset is not None:
            self.syn_dataset = [syn_dataset[0][center], syn_dataset[1][center]]
        else:
            self.syn_dataset = None
        npys_list = [
            e
        ...
        pbar = tqdm(range(len(npys_list)), desc='Processing slides')
        for i in pbar:  
            slide = npys_list[i]
    ...
    def __getitem__(self, idx, path=False):
        ...
        if self.use_stain:
            ...
            X = X_color
        if self.syn_dataset is not None:
            syn_data, syn_label = self.syn_dataset[0], self.syn_dataset[1]
            syn_data_in_label = syn_data[syn_label == y].cpu()
            syn_data_in_label = syn_data_in_label.reshape(-1, syn_data_in_label.size(-1))
            X = torch.cat((X, syn_data_in_label), dim=0)  
        if self.top_k > 0:
    ...
    def get_image(self, c, n_slide, n_patch=0):
        ...
        for i in range(len(idx_slide_n_patches)):
            ...
            if slide_pt.size(0) < n_patch or n_patch == 0:
                ...
                slide_patches = slide_pt[sampled_patch_idx]
